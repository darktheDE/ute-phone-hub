...
function addToCart(productCard) {
    ...
    const productPrice = parseFloat(productCard.querySelector('.price-current').textContent.replace(/[^\d]/g, ''));
...
function updateWishlistDisplay() {
    ...
    if (wishlist.length === 0) {
        wishlistContainer.innerHTML = '<div class="empty-wishlist">Danh sách yêu thích trống</div>';
        return;
    }
    ...
    wishlistContainer.innerHTML = wishlist.map(item => `
        <div class="wishlist-item" data-product-id="${item.id}">
            ...
            <div class="wishlist-item-info">
                ...
                <button class="btn-remove" onclick="removeFromWishlist('${item.id}')">×</button>
            </div>
        </div>
    `).join('');
}
